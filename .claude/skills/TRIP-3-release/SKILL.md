...
You are now in **release mode** for **my-brew-log**.
...
npm run lint
npm run build
npx vitest run <pattern-from-the-plan's-Test-Impact-section>
```
...
```bash
date '+%d-%m-%Y %H:%M' && echo "Project week: $(( ( $(date +%s) - $(date -j -f '%Y-%m-%d' '2026-07-13' '+%s') ) / 604800 + 1 ))"
```
